...
<li><strong>Due: Friday, September 26th at 11pm EST (Baltimore time).</strong></li>
...
The program must also create an output file that reports on each word found that is better than the previous "best". The name of the output file must be "best_input_grid.txt", where "input_grid.txt" is the first command-line argument supplied to the program. After listing each new "best" word found, one per line, the program should also display the overall best word and its point value, exactly as it appeared in the standard output (see above). Lastly you must write the character grid to the file, but with the best word CAPITALIZED to make it easy to visually locate.
